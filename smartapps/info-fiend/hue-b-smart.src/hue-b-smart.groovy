 ...
 *    Version 1.84 Fixed the choose scene/bulbs/groups pages by fixing the redirect link
 */

import groovy.json.*

    definition(
        name: "Hue B Smart (Updated)",
...
preferences {
    ...
    page(name: "chooseBulbs2", content: "chooseBulbs2")
    page(name: "chooseScenes", content: "chooseScenes")
    page(name: "chooseScenes2", content: "chooseScenes2")
    page(name: "chooseGroups", content: "chooseGroups")
    page(name: "chooseGroups2", content: "chooseGroups2")
...
def linkButton(params) {
   ...
   */
    ...
    if (!params.linkDone) {
        ...
        def bridge = getUnlinkedBridges().find {
            it?.key?.contains(params.ssdpUSN)
        }
...
def getVerifiedBridges() {
    getUnlinkedBridges().findAll {
        it?.value?.verified == true
    }
}

def getBridgeBySerialNumber(serialNumber) {
    def b = getUnlinkedBridges().find {
        it?.value?.serialNumber == serialNumber
    }
    if (!b) {
        return getLinkedBridges().find {
            it?.value?.serialNumber == serialNumber
        }
    } else {
        return b
...
String textDonateLink() {
    return "https://www.paypal.me/tmleafs"
}
...
def chooseBulbs(params) {
    makeChooseBulbs(params, "chooseBulbs")
}
def chooseBulbs2(params) {
    makeChooseBulbs(params, "chooseBulbs2")
}
def makeChooseBulbs(params, pageName) {
    def nextPage
    if (pageName == 'chooseBulbs') {
        nextPage = "chooseBulbs2"
    } else {
        nextPage = "chooseBulbs"
    }
    ...
    dynamicPage(name: "chooseBulbs", title: "", install: true) {
        ...
        section("Added Bulbs") {
            addedBulbs.sort {
                ...
                def id = it.value.id
                href(name: "${devId}", page: nextPage, description: "Bulb ID [${id}]", title: "Remove ${name}", params: [mac: params.mac, remove: devId], submitOnChange: true)
            }
        }
        section("Available Bulbs") {
            availableBulbs.sort {
                ...
                def name = it.value.label
                def id = it.value.id
                href(name: "${devId}", page: nextPage, description: "Bulb ID [${id}]", title: "Add ${name}", params: [mac: params.mac, add: it.key], submitOnChange: true)
            }
        }
    }
}
...
def chooseScenes(params) {
    makeChooseScenes(params, "chooseScenes")
}
def chooseScenes2(params) {
    makeChooseScenes(params, "chooseScenes2")
}
def makeChooseScenes(params, pageName) {
    def nextPage
    if (pageName == 'chooseScenes') {
        nextPage = "chooseScenes2"
    } else {
        nextPage = "chooseScenes"
    }
    logMessage("chooseScenes mac: ${params.mac}, add: ${params.add}, remove: ${params.remove}, nextPage is ${nextPage}")
    ...
    dynamicPage(name: pageName, title: "", install: true) {
        logMessage("chooseScenes running.", "info")
        ...
        section("Added Scenes") {
            addedScenes.sort {
                ...
                if (it.value.type == "LightScene") {
                    href(name: "${devId}", page: nextPage, description: "LightScene using Lights ${lights}", title: "Remove ${name}", params: [mac: params.mac, remove: devId], submitOnChange: true)
                } else {
                    href(name: "${devId}", page: nextPage, description: "Group ${it.value.group} using Lights ${lights}", title: "Remove ${name}", params: [mac: params.mac, remove: devId], submitOnChange: true)
                }
            }
        }
        section("Available Scenes") {
            availableScenes.sort {
                it.value.group
            }.each {
                def devId = "${params.mac}/SCENE${it.key}"
                def name = it.value.label
                def lights = it.value.lights
                if (it.value.type == "LightScene") {
                    href(name: "${devId}", page: nextPage, description: "LightScene Using Lights ${lights}", title: "Add ${name}", params: [mac: params.mac, add: it.key], submitOnChange: true)
                } else {
                    href(name: "${devId}", page: nextPage, description: "Group ${it.value.group} Using Lights ${lights}", title: "Add ${name}", params: [mac: params.mac, add: it.key], submitOnChange: true)
                }
            }
        }
    }
...
def chooseGroups(params) {
    makeChooseGroups(params, "chooseGroups")
}
def chooseGroups2(params) {
    makeChooseGroups(params, "chooseGroups2")
}
def makeChooseGroups(params, pageName) {
    def nextPage
    if (pageName == 'chooseGroups') {
        nextPage = "chooseGroups2"
    } else {
        nextPage = "chooseGroups"
    }
    ...
    return dynamicPage(name: "chooseGroups", title: "", install: false, uninstall: false, nextPage: "manageBridge") {
        ...
        section("Hue Groups Added to SmartThings") {
            addedGroups.sort {
                ...
                def devId = "${params.mac}/GROUP${it.key}"
                def name = it.value.label
                href(name: "${devId}", page: nextPage, description: "GroupID-${it.key}", title: "Remove ${name}", params: [mac: params.mac, remove: devId], submitOnChange: true)
            }
        }
        section("Available Hue Groups") {
            availableGroups.sort {
                ...
                def devId = "${params.mac}/GROUP${it.key}"
                def name = it.value.label
                href(name: "${devId}", page: nextPage, description: "GroupID-${it.key}", title: "Add ${name}", params: [mac: params.mac, add: it.key])
            }
        }
    }
}
        ...
        if (devId.contains(mac) && devId.contains("/")) {
            if (it.deviceNetworkId.contains("BULB")) {
                ...
                if (bBulb != null) { // If user removes bulb from hue without removing it from smartthings,
                    ...
                    if (type.equalsIgnoreCase("Dimmable light") || type.equalsIgnoreCase("Dimmable plug-in unit")) {
                        ...
                        ["bri", "ct", "reachable", "on"].each {
                            p ->
                            it.updateStatus("state", p, bridge.value.bulbs[bulbId].state[p])
                        }
                    } else if (type.equalsIgnoreCase("Color Light")) {
                        ["reachable", "on", "bri", "hue", "sat", "xy", "effect", "colormode"].each {
                            p ->
                            it.updateStatus("state", p, bridge.value.bulbs[bulbId].state[p])
                        }
                    } else {
                        ["reachable", "on", "bri", "hue", "sat", "ct", "xy", "effect", "colormode"].each {
                            p ->
                            it.updateStatus("state", p, bridge.value.bulbs[bulbId].state[p])
                        }
                    }
            ...
            if (it.deviceNetworkId.contains("GROUP")) {
                ...
                if (groupFromBridge != null) { // If user removes group from hue without removing it from smartthings, 
                    ...
                    if (colormode != null) {
                        ["on", "bri", "sat", "ct", "xy", "effect", "hue", "colormode"].each {
                            p ->
...
private verifyHueBridges() {
    def devices = getUnlinkedBridges().findAll {
        it?.value?.verified != true
    }
    devices.each {
        def ip = convertHexToIP(it.value.networkAddress)
...
private processDiscoveryResponse(parsedEvent) {
    ...
    def bridge = getUnlinkedBridges().find {
        it?.key?.contains(parsedEvent.ssdpUSN)
    }
    if (!bridge) {
        bridge = getLinkedBridges().find {
            it?.key?.contains(parsedEvent.ssdpUSN)
        }
    }
    if (bridge) {
...
private sendDeveloperReq(mac) {
    ...
    ], "${selectedHue}", [callback: "usernameHandler"]))
}
